...
local is_password, is_encrypted, is_level, is_extract, cmd_password, cmd_level, default_extension = false, false, false, false, "", "", "zip"
...
-- Table of archive commands (compression)
...
-- Table of extract commands (newly added)
local extract_commands = {
    ["%.zip$"] = {
        {command = "unzip", args = {}, password_arg = "-P", passwordable = true},
        {command = {"7z", "7zz", "7za"}, args = {"x"}, password_arg = "-p", passwordable = true}
    },
    ["%.7z$"] = {
        {command = {"7z", "7zz", "7za"}, args = {"x"}, password_arg = "-p", passwordable = true}
    },
    ["%.rar$"] = {
        {command = "unrar", args = {"x"}, password_arg = "-p", passwordable = true},
        {command = {"7z", "7zz", "7za"}, args = {"x"}, password_arg = "-p", passwordable = true}
    },
    ["%.tar.gz$"] = {
        {command = {"tar", "bsdtar"}, args = {"-xzf"}},
        {command = {"7z", "7zz", "7za"}, args = {"x"}, password_arg = "-p", passwordable = true}
    },
    ["%.tar.xz$"] = {
        {command = {"tar", "bsdtar"}, args = {"-xJf"}},
        {command = {"7z", "7zz", "7za"}, args = {"x"}, password_arg = "-p", passwordable = true}
    },
    ["%.tar.bz2$"] = {
        {command = {"tar", "bsdtar"}, args = {"-xjf"}},
    ...
    entry = function(_, job)
        ...
        if job.args ~= nil then
            for _, arg in ipairs(job.args) do
                if arg:match("^%-(%w+)$") then
                    -- Handle combined flags (e.g., -phl, -dp for extract with password)
                    for flag in arg:sub(2):gmatch(".") do
                        ...
                        elseif flag == "l" then
                            is_level = true
                        elseif flag == "d" then  -- New: extract flag
                            is_extract = true
                        end
                    end
        ...
        if #fnames == 0 then
            notify_error("No files selected", "error")
            return
        end

        -- Extract mode (newly added)
        if is_extract then
            -- Get extract directory
            local extract_dir, event = ya.input({
                title = "Extract to directory:",
                position = {"top-center", y = 3, w = 40},
                value = output_dir  -- Default to current directory
            })
            if event ~= 1 then
                return
            end
            ...
            if not is_valid_filename(extract_dir) then
                notify_error("Invalid extract directory name", "error")
                return
            end
            local extract_url = Url(combine_url(output_dir, extract_dir))
            
            -- Create extract directory if not exists
            local dir_status, dir_err = fs.create("dir_all", extract_url)
            if not dir_status then
                notify_error(string.format("Failed to create extract directory: %s", dir_err), "error")
                return
            end

            -- Process each selected archive file
            for filepath, filenames in pairs(path_fnames) do
                for _, filename in ipairs(filenames) do
                    local archive_path = combine_url(filepath, filename)
                    local matched = false
                    local extract_cmd, extract_args, password_arg, passwordable

                    -- Find matching extract command
                    for pattern, cmd_list in pairs(extract_commands) do
                        if filename:match(pattern) then
                            matched = true
                            for _, cmd in ipairs(cmd_list) do
                                local find_cmd = type(cmd.command) == "table" and find_command_name(cmd.command) or cmd.command
                                if is_command_available(find_cmd) then
                                    extract_cmd = find_cmd
                                    extract_args = cmd.args
                                    password_arg = cmd.password_arg or ""
                                    passwordable = cmd.passwordable or false
                                    break
                                end
                            end
                            if extract_cmd then break end
                        end
                    end

                    if not matched then
                        notify_error(string.format("Unsupported archive format: %s", filename), "warn")
                        goto continue  -- Skip to next file
                    end

                    if not extract_cmd then
                        notify_error(string.format("No extractor found for: %s", filename), "warn")
                        goto continue
                    end

                    -- Handle password
                    local final_args = {unpack(extract_args)}
                    if is_password and passwordable then
                        local pwd, event = ya.input({
                            title = "Enter password for " .. filename,
                            obscure = true,
                            position = {"top-center", y = 3, w = 40}
                        })
                        if event ~= 1 then return end
                        table.insert(final_args, password_arg .. pwd)
                    end

                    -- Add archive path and output directory
                    table.insert(final_args, archive_path)
                    table.insert(final_args, "-d")  -- Most tools use -d for output directory
                    table.insert(final_args, tostring(extract_url))

                    -- Execute extract command
                    local status, err = Command(extract_cmd)
                        :arg(final_args)
                        :cwd(filepath)
                        :spawn()
                        :wait()
                    ...
                    if not status or not status.success then
                        notify_error(string.format("Failed to extract %s: %s", filename, err), "error")
                    else
                        ya.notify({
                            title = "Extract Success",
                            content = string.format("Extracted %s to %s", filename, extract_dir),
                            level = "info",
                            timeout = 3
                        })
                    end
            ...
            return  -- Exit after extract operation
        end

        -- Original compression logic (unchanged)
